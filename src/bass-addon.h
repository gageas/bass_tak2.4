...
typedef HSTREAM (CALLBACK STREAMCREATEURLPROC)(const char *url, DWORD offset, DWORD flags, DOWNLOADPROC *proc, void *user); // BASS_StreamCreateURL plugin function (unsupported protocol)
...
typedef struct {
	...
	struct { // file functions
		...
		void (WINAPI *SetFlags)(BASSFILE file, DWORD flags); // set BASSFILE_xxx flags (BASSFILE_RESTRICT only)
...
#define BASSFILE_RESTRICT	BASS_STREAM_RESTRICT
